...
std::vector<char> possible_right_extension(uint64_t mer,int k,std::unordered_map<uint64_t,bool> &kmers_to_add_to_graph,Index& ind,GfaGraph& graph,std::unordered_map<uint64_t,bool>& candidate_splits,uint64_t *possible_join){
    /*
...
std::string extend_right(std::string kmer,uint64_t mer, Index& unitigs_index,std::unordered_map<uint64_t,bool> &kmers_to_add_to_graph,GfaGraph& graph,std::unordered_map<uint64_t,bool>& candidate_splits,std::unordered_map<int,std::pair<int,int>> &candidate_join,int id,bool extended_from_right,std::unordered_map<int,std::pair<int,int>> &funitigs_joins){
   ...
   */
   std::vector<char> possible_character_to_extend=possible_right_extension(suffix,unitigs_index.get_k(),kmers_to_add_to_graph,unitigs_index,graph,candidate_splits,&possible_join);
   std::vector<char> possible_character_to_extend_left=possible_right_extension(reverse_complement(suffix,unitigs_index.get_k()-1),unitigs_index.get_k(),kmers_to_add_to_graph,unitigs_index,graph,candidate_splits,&possible_join_2);
...
std::string unitig_from_kmers_string(std::string kmer,uint64_t mer,Index& unitig_index, std::unordered_map<uint64_t,bool>& kmers,GfaGraph& graph,int id,std::unordered_map<uint64_t,bool> &candidate_splits,std::unordered_map<int,std::pair<int,int>> &candidate_join,std::unordered_map<int,std::pair<int,int>> &funitigs_joins){
    ...
    std::string left=reverseComplement(extend_right(reverseComplement(kmer),reverse_complement(mer,unitig_index.get_k()),unitig_index,kmers,graph,candidate_splits,candidate_join,id,false,funitigs_joins));//extend the left, extend right of reverse then reverse the result
...
Unitig unitig_from_kmers(uint64_t kmer,Index &unitig_index, std::unordered_map<uint64_t,bool> &kmers,GfaGraph& graph,int id,std::unordered_map<uint64_t,bool> &candidate_splits,std::unordered_map<int,std::pair<int,int>> &candidate_join,std::unordered_map<int,std::pair<int,int>> &funitigs_joins){
    /*
    ...
    */
    return Unitig(unitig_from_kmers_string(to_string(kmer,unitig_index.get_k()),kmer,unitig_index,kmers,graph,id,candidate_splits,candidate_join,funitigs_joins));
}
...
void update_CdBG(Index& graph_index,GfaGraph& graph,std::unordered_map<int,Unitig>& constructed_unitigs,std::unordered_map<uint64_t,bool> &candidate_splits,std::unordered_map<int,std::pair<int,int>> &candidate_join,bool verbose,bool update_index){
    ...
    for(int i=0;i<sorted_splits.size();i++){
        ...
        if(i<sorted_splits.size()-1){
            ...
            if(next_id==current_id){
                next_pos=next_split&&0x7FFFFFFF;
            }
        }
        if(first_split){
            first_split=false;
            Unitig left=Unitig(left_unused,3-(pos+graph_index.get_k()-2+(int)(left_unused))%4,std::vector<uint8_t>(seq_bases.begin(),seq_bases.begin()+1+static_cast<int>(std::floor((pos +graph_index.get_k()-2+ (int)(left_unused))/4))));
        ...
        if(next_pos!=0){// take from pos to next_pos+(k-1)
            Unitig middle=Unitig((pos+(int)(left_unused))%4,3-(next_pos+graph_index.get_k()-2+(int)(left_unused))%4,std::vector<uint8_t>(seq_bases.begin()+static_cast<int>(std::floor((pos + (int)(left_unused))/4)),seq_bases.begin()+1+static_cast<int>(std::floor((next_pos+graph_index.get_k()-2+ (int)(left_unused))/4))));
            ...
            if(update_index){
                graph_index.update_unitig(seq,new_id,current_id,pos,seq.unitig_length()-graph_index.get_k(),true);
            }
            new_id++;
        }
    }
